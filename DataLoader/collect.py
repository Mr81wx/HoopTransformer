...
def create_batch_A(batch): #单向轨迹重建
    ...
    for scene in batch:
        ...
        states_hidden_BP[:,:4] = False #防止整段都是高速轨迹
        '''
...
#collect_fn 函数
def create_batch_B(batch): #单向轨迹重建
...
def create_batch_B2(batch): #单向轨迹重建
...
def create_batch_C(batch): #单向轨迹重建
...
def create_batch_D(batch): #单向轨迹重建
    #tensor序列长度 11*121*3 #补齐T维度 到121
    sample_freq = 5 # 帧数
    sequence_length = int(24*25/sample_freq + 1) # 24s 对应的序列长度
    list_24s = [i*(1/sample_freq) for i in range(0, sequence_length)] # sequence_length 对应的game clock
    ...
    for scene in batch:
        ...
        #bool型，(11,121) True为padding
        states_padding = states_feat[:,:,0]
        states_padding = states_padding == 0 #True代表填充的位置

        #
        #mask process # True代表hidden的位置
        states_hidden = create_random_tensor(scene_tensor.shape[0], scene_tensor.shape[1], 0.7)
        states_hidden_BP = torch.nn.functional.pad(states_hidden,  (head_padding_size+1, end_padding_size, 0, 0), mode='constant', value=True)
        states_hidden_BP[:,1:4] = False
